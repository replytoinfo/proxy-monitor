...
- **Подтверждает отказ запасным адресом** — прокси объявляется `down` только после провала основного и запасного адреса (по умолчанию Cloudflare и Google, разные компании — общий отказ маловероятен). Запасная проверка идёт только после провала основной, так что здоровые прокси лишнего трафика не получают.
- **Следит за ротацией IP** — для мобильных прокси: если внешний IP не менялся дольше `ROTATION_MAX_AGE`, приходит алерт. Эхо-сервисов несколько, перебор до первого успеха.
- **Не молчит о собственных сбоях** — провал IP-пробы виден отдельным алертом, а не тишиной. Внешний watchdog (healthchecks.io) подтверждает, что процесс жив.
- **Схлопывает шквал** — при падении всех прокси разом уходит одно сообщение, а не N.
...
Пароли прокси хранятся зашифрованными (AES-256-GCM, ключ в `ENCRYPTION_KEY`). Пароли не попадают в текст ошибок, `HEALTHCHECK_URL` не логируется никогда — он содержит секрет. Проверяемые адреса проходят через политику `net-policy.ts`: обращения в приватные диапазоны блокируются, если явно не разрешены через `ALLOW_PRIVATE_TARGETS`.
